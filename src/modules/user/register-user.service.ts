...
export class RegisterUserService {
  constructor(
    ...
    private readonly susbscriptionRepository: ISubscriptionRepository,
  ...
  async exec({
    ...
    const doesTheEmailAlreadyExist = await this.userRepository.findByEmail(
      prisma,
      email,
    );
    ...
    const currentSubscription =
      await this.susbscriptionRepository.findActiveByClinicId(prisma, clinicId);

    if (currentSubscription && currentSubscription.status !== "ACTIVE") {
      throw new UnauthorizedError(
        "Subscription is not active or in a trial mode",
      );
    }

    const currentPlan = await this.planRepository.findPlanById(
      prisma,
      currentSubscription!.plan_id,
    );
    const clinicMembersCount =
      await this.clinicMemberRepository.countMembersByClinicId(
        prisma,
        clinicId,
      );
    ...
    if (
      ...
      currentPlan.max_users <= clinicMembersCount
    ) {
